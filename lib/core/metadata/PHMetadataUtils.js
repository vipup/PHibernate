...
class PHMetadataUtils {
    static getRelatedOneToManyConfig(manyToOnePropertyName, qEntity) {
        ...
        return MetadataUtils_1.MetadataUtils.getRelatedOneToManyConfig(manyToOnePropertyName, entityMetadata);
    }
    ...
    static getIdValue(entityObject, qEntity) {
        let entityMetadata = qEntity.__entityConstructor__;
        return MetadataUtils_1.MetadataUtils.getIdValue(entityObject, entityMetadata);
    }
    static getTableName(qEntity) {
        let entityMetadata = qEntity.__entityConstructor__;
        let tableName = MetadataUtils_1.MetadataUtils.getTableName(entityMetadata);
        if (!tableName) {
    ...
    static getOneToManyConfig(propertyName, qEntity) {
        let entityMetadata = qEntity.__entityConstructor__;
        return MetadataUtils_1.MetadataUtils.getOneToManyConfig(propertyName, entityMetadata);
    }
}
