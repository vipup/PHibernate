...
class EntityManager {
    ...
    deleteWhere(entityClass, phRawDelete) {
        return __awaiter(this, void 0, void 0, function* () {
            let qEntity = PH_1.PH.getQEntityFromEntityClass(entityClass);
            let phSqlDelete = this.getPHSQLDelete(qEntity, phRawDelete);
            let changeRecord = yield this.localStore.deleteWhere(qEntity.__entityName__, phSqlDelete, this.localStore.activeChangeGroup);
            return changeRecord.numberOfAffectedRecords;
        });
    }
    getPHSQLDelete(qEntity, phRawDelete) {
        ...
        let phSqlDelete = new querydsl_typescript_1.PHSQLDelete(phRawDelete, qEntity, qEntityMap, entitiesRelationPropertyMap, entitiesPropertyTypeMap);
        return phSqlDelete;
    }
    ...
    updateWhere(entityClass, phRawUpdate) {
        return __awaiter(this, void 0, void 0, function* () {
            let qEntity = PH_1.PH.getQEntityFromEntityClass(entityClass);
            let phSqlUpdate = this.getPHSQLUpdate(qEntity, phRawUpdate);
            let changeRecord = yield this.localStore.updateWhere(qEntity.__entityName__, phSqlUpdate, this.localStore.activeChangeGroup);
            return changeRecord.numberOfAffectedRecords;
        });
    }
    getPHSQLUpdate(qEntity, phRawUpdate) {
        let qEntityMap = PH_1.PH.qEntityMap;
        let entitiesRelationPropertyMap = PH_1.PH.entitiesRelationPropertyMap;
        let entitiesPropertyTypeMap = PH_1.PH.entitiesPropertyTypeMap;
        let phSqlUpdate = new querydsl_typescript_1.PHSQLUpdate(phRawUpdate, qEntity, qEntityMap, entitiesRelationPropertyMap, entitiesPropertyTypeMap);
        return phSqlUpdate;
    }
