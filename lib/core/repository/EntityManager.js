...
class EntityManager {
    constructor(config) {
        ...
        if (deltaStoreConfig) {
            ...
            if (deltaStoreConfig.offlineDeltaStore) {
                let offlineSharingAdaptor = OfflineStore_1.getOfflineSharingAdaptor(deltaStoreConfig.offlineDeltaStore.type);
                this.offlineDeltaStore = new DeltaStore_1.OfflineDeltaStore(deltaStoreConfig.offlineDeltaStore, offlineSharingAdaptor);
            }
        }
        if (config.localStoreConfig) {
            this.localStore = LocalStore_1.getLocalStoreAdaptor(config.localStoreConfig.setupInfo.type, config.localStoreConfig.setupInfo.idGeneration);
        }
    }
    ...
    persistEntity(entityClass, entity, operation) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.localStore['create']('ChangeGroup', this.localStore.activeChangeGroup, new ChangeGroup_1.StubChangeGroup());
            ...
            if (this.isOnline()) {
                ...
                yield this.offlineDeltaStore.addChange(this.deltaStore.config.offlineDeltaStore.config, changeGroup);
            }
            return entity;
        });
    }
